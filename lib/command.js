...
var LiveScript, path, fs, util, prelude, each, breakList, keys, filter, dasherize, map, lines, unlines, take, ref$, parseOptions, generateHelp, nameFromPath, SourceNode, version, slice$ = [].slice, toString$ = {}.toString;
LiveScript = require('..');
path = require('path');
fs = require('fs');
util = require('util');
prelude = require('prelude-ls'), each = prelude.each, breakList = prelude.breakList, keys = prelude.keys, filter = prelude.filter, dasherize = prelude.dasherize, map = prelude.map, lines = prelude.lines, unlines = prelude.unlines, take = prelude.take;
...
(function(it){
  ...
  var ref$, say, sayWithTimestamp, warn, die, p, pp, ppp, dasherizeVars, startsWith, fileExists, o, positional, e, validMapValues, toInsert, that, filename, jsonCallback;
  ref$ = arg$ != null
    ? arg$
  ...
  function compileScript(filename, input, base){
    ...
    try {
      ...
      if (run) {
        ...
        t.result = LiveScript.run(t.output.toString(), options, {
          js: true,
      ...
      if (o.print || !filename) {
        say((toString$.call(t.output).slice(8, -1) === 'String'
          ? t.output
